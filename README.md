...
这个项目是我学习操作系统内存管理的实践，实现了几种常见的内存分配算法。不依赖标准库的 malloc/free，全部通过 mmap 系统调用直接和操作系统打交道。
...
这个项目让我对操作系统的内存管理有了更深的理解。从最开始不知道怎么不用 malloc 分配内存，到现在能实现几种不同策略的分配器，收获很大。

代码可能还有不少可以优化的地方，欢迎交流。
